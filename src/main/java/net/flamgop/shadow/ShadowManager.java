...
public class ShadowManager {
    ...
    public static Matrix4f getShadowMatrix(Camera camera, DirectionalLight light) {
        Vector3f lightDir = new Vector3f(light.direction).normalize();
        Matrix4f lightView = new Matrix4f().lookAt(new Vector3f(lightDir).mul(-100), lightDir, new Vector3f(0,1,0));
        ...
        computeOrthoExtents(lightView, invCamViewProj, orthoMin, orthoMax);
        Vector3f min = new Vector3f(), max = new Vector3f();
        invCamViewProj.frustumAabb(min, max);
        Vector2f nearAndFar = computeNearAndFar(lightView, orthoMin, orthoMax, new AABB(min, max));
